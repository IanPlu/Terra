...
from terra.event import TEXT_SUBMIT_INPUT, TEXT_CANCEL_INPUT, publish_game_event
from terra.keybindings import KB_CONFIRM, KB_MENU
...
class TextInput(GameObject):
    ...
    def submit_text(self):
        if len(self.current_text) > 0:
            publish_game_event(TEXT_SUBMIT_INPUT, {
                "tag": self.tag,
    ...
    def cancel_text(self):
        publish_game_event(TEXT_CANCEL_INPUT, {})
    ...
    def step(self, event):
        ...
        if event.type == KEYDOWN:
            if event.key in KB_CONFIRM:
                ...
                self.cancel_text()
            elif event.key == K_BACKSPACE and len(self.current_text) > 0:
